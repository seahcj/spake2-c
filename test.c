...
int main() {
    ...
    uint8_t aMessage[SPAKE2_MAX_MSG_SIZE];
    uint8_t bMessage[SPAKE2_MAX_MSG_SIZE];
    size_t aMsgSize;
    size_t bMsgSize;

    int status = SPAKE2_generate_msg(alice, aMessage, &aMsgSize, SPAKE2_MAX_MSG_SIZE, kPassword, sizeof(kPassword));
    status += SPAKE2_generate_msg(bob, bMessage, &bMsgSize, SPAKE2_MAX_MSG_SIZE, kPassword, sizeof(kPassword));
    ...
    hexify(aMessage, SPAKE2_MAX_MSG_SIZE, output, 65);
    printf("ALICE(%zu) ==> %s\n", aMsgSize, output);
    hexify(bMessage, SPAKE2_MAX_MSG_SIZE, output, 65);
    printf("BOB(%zu)   ==> %s\n", bMsgSize, output);

    size_t aKeyLen = 0;
    uint8_t aKey[SPAKE2_MAX_KEY_SIZE];
    status = SPAKE2_process_msg(alice, aKey, &aKeyLen, sizeof(aKey), (uint8_t *) bMessage, bMsgSize);

    size_t bKeyLen = 0;
    uint8_t bKey[SPAKE2_MAX_KEY_SIZE];
    ...
    hexify(aKey, SPAKE2_MAX_KEY_SIZE, output, 129);
    printf("ALICE(%zu) <== %s\n", aKeyLen, output);
    hexify(bKey, SPAKE2_MAX_KEY_SIZE, output, 129);
    printf("BOB(%zu)   <== %s\n", bKeyLen, output);
    return 0;
}
